...
EMERGENCY_CSS = """
<style>
body.emergency-mode .stApp { border-top: 3px solid #ef4444 !important; }
body.emergency-mode::before {
    content: ""; position: fixed; top: 0; left: 0; right: 0; height: 3px;
    background: #ef4444;
    z-index: 999998; pointer-events: none;
}
body.emergency-mode .glass-card-critical, body.emergency-mode .emergency-pulse { border-left: 3px solid #ef4444; background: rgba(239, 68, 68, 0.06); }
.emergency-warning-indicator { display: inline-flex; align-items: center; gap: 6px; opacity: 1; }
.critical-incident-banner {
    position: relative; background: rgba(127,7,7,0.92);
    border: 1px solid rgba(239,68,68,0.6); border-left: 4px solid #ef4444; border-radius: 10px; padding: 12px 20px; margin-bottom: 12px;
...
.cctv-overlay-live { background: rgba(239,68,68,0.85); color: #fff; font-size: 9px; font-weight: 800; padding: 2px 8px; border-radius: 3px; letter-spacing: 1px; text-transform: uppercase; opacity: 1; }
...
.cctv-overlay-rec-dot { width: 7px; height: 7px; border-radius: 50%; background: #ef4444; opacity: 1; }
...
.cctv-focus-mode { transform: scale(1.05); transform-origin: center center; transition: transform 0.3s ease; border: 2px solid rgba(239,68,68,0.8); border-radius: 8px; z-index: 5; position: relative; }
.kpi-value-changed { transform: scale(1); }
...
.timeline-entry { opacity: 1; }
.timeline-resolved { opacity: 0.6; }
.ai-decision-update { opacity: 1; }
.compound-risk-new { opacity: 1; max-height: 200px; }
...
.copilot-section { opacity: 1; }
.alert-pinned { border: 1px solid rgba(239,68,68,0.4) !important; }
.alert-entering { opacity: 1; max-height: 100px; }
.alert-fading { opacity: 0.3; filter: grayscale(0.6); }
.emergency-response-expanded { max-height: 600px; overflow: hidden; }
.emergency-response-collapsed { max-height: 0; overflow: hidden; }
body.emergency-resolving .stApp { opacity: 1; }
.ambient-intensify { filter: brightness(1); }
</style>
"""
...
def render_live_timeline(events: List[Dict[str, Any]], is_resolved: bool = False) -> str:
    ...
    for i, e in enumerate(events):
        ...
        events_html.append(f'<div class="{entry_cls}" style="display:grid;grid-template-columns:64px 20px 1fr auto;align-items:center;gap:6px;padding:5px 10px;margin-bottom:3px;background:{bg};border:1px solid {border};border-left:3px solid {color};border-radius:6px;"><span style="color:#64748b;font-family:monospace;font-size:9px;text-align:left;">{time_str}</span><span style="font-size:11px;text-align:center;">{icon}</span><span style="color:#cbd5e1;font-weight:500;font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{message}</span><span style="background:{bg};color:{color};border:1px solid {border};border-radius:4px;padding:1px 6px;font-size:7.5px;font-weight:800;text-transform:uppercase;letter-spacing:0.4px;">{status_label}</span></div>')
    timeline_html = f'<div style="background:rgba(17,24,39,0.7);border:1px solid var(--border2);border-left:3px solid #3b82f6;border-radius:12px;padding:14px 16px;min-height:170px;box-sizing:border-box;font-family:Outfit,sans-serif;display:flex;flex-direction:column;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><span style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:1px;font-weight:700;">LIVE INCIDENT TIMELINE</span><span style="background:rgba(59,130,246,0.15);color:#3b82f6;border:1px solid rgba(59,130,246,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">LOG ACTIVE</span></div><div style="max-height:160px;overflow-y:auto;font-size:10px;line-height:1.4;flex-grow:1;">{"".join(events_html)}</div></div>'
...
def render_enhanced_safety_copilot(
    ...
    if level not in ('HIGH', 'CRITICAL'):
        return clean_html('<div style="background:rgba(34,197,94,0.04);border:1px solid #22c55e44;border-left:3px solid #22c55e;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:#8b5cf6;font-size:11px;font-weight:600;letter-spacing:1px;">🤖 AI SAFETY COPILOT</span><span style="background:rgba(34,197,94,0.15);color:#22c55e;border:1px solid rgba(34,197,94,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">MONITORING</span></div><div style="color:#22c55e;font-size:10px;">✅ All systems nominal — no incident explanation required.</div></div>')
...
def render_enhanced_emergency_response(data_dict: Dict[str, Any], selected_zone: str) -> str:
    ...
    if not is_active:
        return clean_html('<div style="background:rgba(34,197,94,0.04);border:1px solid #22c55e44;border-left:3px solid #22c55e;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:1px;">🆘 EMERGENCY RESPONSE — STANDBY</span><span style="background:rgba(34,197,94,0.15);color:#22c55e;border:1px solid rgba(34,197,94,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">STANDBY</span></div><div style="color:#22c55e;font-size:10px;">✅ No active emergency — response teams on standby</div></div>')
...
def render_enhanced_compound_risk(
    ...
    if compound_score == 0:
        return clean_html('<div style="background:rgba(34,197,94,0.04);border:1px solid #22c55e44;border-left:3px solid #22c55e;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:1px;">🧠 COMPOUND RISK INTELLIGENCE</span><span style="background:rgba(34,197,94,0.15);color:#22c55e;border:1px solid rgba(34,197,94,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">STABLE</span></div><div style="color:#22c55e;font-size:10px;">✅ No compound risks detected — all zones stable</div></div>')
...
def render_enhanced_smart_alerts(
    ...
    from src.ui_components import clean_html
    if not st.session_state.get('sim_play_active', False):
        return clean_html('<div style="background:rgba(30,41,59,0.3);border:1px solid #1e3a5f;border-left:3px solid #64748b;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:1px;">🚨 SMART ALERT PRIORITIZATION</span><span style="background:rgba(100,116,139,0.15);color:#94a3b8;border:1px solid rgba(100,116,139,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">STANDBY</span></div><div style="color:#64748b;font-size:10px;">Simulation inactive — no alerts to prioritize.</div></div>')
    ...
    if not zone_alerts:
        return clean_html('<div style="background:rgba(34,197,94,0.04);border:1px solid #22c55e44;border-left:3px solid #22c55e;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:1px;">🚨 SMART ALERT PRIORITIZATION</span><span style="background:rgba(34,197,94,0.15);color:#22c55e;border:1px solid rgba(34,197,94,0.3);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">MONITORING</span></div><div style="color:#22c55e;font-size:10px;">✅ No active alerts — all zones nominal</div></div>')
    ...
    for i, alert in enumerate(sorted_alerts[:5]):
        ...
        message = getattr(alert, 'message', ''); zone = getattr(alert, 'zone', ''); is_pinned = i == 0
        pin_badge = "📌 PINNED" if is_pinned else f"#{i+1}"
        item_bg = f"rgba(239,68,68,0.06)" if sev_name == "CRITICAL" else (f"rgba(245,158,11,0.05)" if sev_name in ("HIGH", "MEDIUM") else "rgba(34,197,94,0.04)")
        alert_items += f'<div style="background:{item_bg};border-radius:6px;padding:6px 10px;margin-bottom:4px;border:1px solid {sev_color}40;border-left:3px solid {sev_color};"><div style="display:flex;justify-content:space-between;align-items:center;"><span style="color:{sev_color};font-size:10px;font-weight:700;">{pin_badge} {sev_name} • {zone}</span><span style="background:{sev_color}20;color:{sev_color};border:1px solid {sev_color}40;font-size:7.5px;font-weight:800;padding:1px 5px;border-radius:3px;text-transform:uppercase;">{sev_name}</span></div><div style="color:#94a3b8;font-size:9px;margin-top:2px;">{message}</div></div>'
    html = f"""<div style="background:rgba(239,68,68,0.04);border:1px solid #ef444444;border-left:3px solid #ef4444;border-radius:12px;padding:10px 14px;font-family:Outfit,sans-serif;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;"><span style="color:#94a3b8;font-size:11px;font-weight:600;letter-spacing:1px;">🚨 SMART ALERT PRIORITIZATION — {len(sorted_alerts)} Ranked Alert(s)</span><span style="background:rgba(239,68,68,0.2);color:#ef4444;border:1px solid rgba(239,68,68,0.4);font-size:8px;font-weight:700;padding:1px 6px;border-radius:4px;text-transform:uppercase;">ALERTS ACTIVE</span></div>{alert_items}</div>"""
    return clean_html(html)
